...
``super-convergence'' (SC) can be achieved, where ResNets \cite{ResNet} can be
...
\begin{itemize}
    ...
    \item Show that SC occurs on various ResNet depths
...
had to choose which experiments from which we could glean the most.
...
code on GitHub. We chose to not look at this code and to reimplement their
...
test as well. If they were not used with PCLR, then this could explain why CLR
has an advantage in all of their tests.
...
processor, 16GB of RAM, and an Nvidia GeForce GTX 1070 GPU. The effect of batch
sizes on our results will be discussed in section \ref{sec:conclusion}.
